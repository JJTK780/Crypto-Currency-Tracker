...
export default function PriceAlertList() {
  ...
  if (alerts.length === 0) {
    return <div className="text-center p-4 text-gray-600 dark:text-gray-400">No price alerts set.</div>;
  }

  return (
    <div className="space-y-4 p-4 border rounded-md shadow-sm border-gray-200 dark:border-gray-800">
      {alerts.map((alert) => (
        <div
          key={alert.id}
          className="flex items-center justify-between p-3 border rounded-md border-gray-200 dark:border-gray-700"
        >
          <div>
          ...
          <div className="flex items-center space-x-2">
            <button
              ...
              className={`px-2 py-1 rounded-md text-white ${alert.active ? "bg-emerald-500 hover:bg-emerald-600" : "bg-gray-500 hover:bg-gray-600"
                }`}
